...
import org.example.entities.HistoricoBloqueios;
...
public class DaoJanelasBloqueadasImple implements org.example.dao.DaoJanelasBloqueadas {
    ...
    public List<HistoricoBloqueios> visualizarHistoricoBloqueados(Maquina maquina) {
        List<HistoricoBloqueios> listaHistoricoBloqueios = new ArrayList<>();
        HistoricoBloqueios historicoBloqueios = new HistoricoBloqueios();
            ...
            ps = conn.prepareStatement("""
                    SELECT
                       ...
                       GROUP BY
                         ...
                         c.nome;
                    """);
            ps.setInt(1, maquina.getIdSetor());
            rs = ps.executeQuery();
            while (rs.next()) {
                historicoBloqueios.setNome(rs.getString("nome"));
                historicoBloqueios.setQuantidade_bloqueios(rs.getInt("quantidade_bloqueios"));
                listaHistoricoBloqueios.add(historicoBloqueios);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return listaHistoricoBloqueios;
    }
